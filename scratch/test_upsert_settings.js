...
async function run() {
  ...
  const dataToSave = { company_name: "Test2", banner_url: "Test2" };
  const { data, error } = await supabase.from('settings').upsert({ key, value: JSON.stringify(dataToSave) }, { onConflict: 'key' });
