...
async function run() {
  ...
  const { data: uploadData, error: uploadError } = await supabase.storage.from('images').upload('test.txt', testFile, { upsert: true });
  ...
  const { data: settingsData, error: settingsError } = await supabase.from('settings').upsert({ key, value: JSON.stringify(dataToSave) }, { onConflict: 'key' });
